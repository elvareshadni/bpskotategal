...
<div class="card mb-3">
    <div class="card-body row g-3 align-items-end">
        ...
        <div class="col-md-3 ms-auto text-end">
            <a href="<?= base_url('admin/indicator/form'); ?>" class="btn btn-primary">Tambah Indikator</a>
        </div>
    </div>
</div>
...
<script>
    (async function() {
        ...
        async function loadList() {
           ...
           <form action="<?= base_url('admin/subindicator/delete'); ?>/${s.id}" method="post" class="d-inline">
             <?= csrf_field(); ?>
             <button class="btn btn-link text-danger p-0 ms-2" onclick="return confirm('Hapus subindikator?')">hapus</button>
           </form>
         </li>`).join('');
        ...
        <div class="border rounded p-3 mb-3">
          <div class="d-flex align-items-center">
            ...
            <div class="ms-auto">
              <form action="<?= base_url('admin/indicator/delete'); ?>/${it.id}" method="post" class="d-inline">
                <?= csrf_field(); ?>
                <button class="btn btn-outline-danger btn-sm" onclick="return confirm('Hapus indikator beserta datanya?')">Hapus</button>
              </form>
            </div>
          </div>
